...
class ReconciliationPackagingTests(unittest.TestCase):
    def test_comparison_sql_keeps_legacy_documento_alt_suffix_match(self) -> None:
        self.assertIn("WHERE mov.id_terc = %s", COMPARISON_SQL)
        self.assertIn("TRIM(mov.doc_inv_tipo) = 'EA'", COMPARISON_SQL)
        self.assertIn("%s LIKE '%%' || TRIM(mov.documento_alt)", COMPARISON_SQL)
